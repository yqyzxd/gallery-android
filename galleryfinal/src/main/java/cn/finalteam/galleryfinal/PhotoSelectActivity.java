...
public class PhotoSelectActivity extends PhotoBaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener{
    ...
    public void onClick(View v) {
        ...
        if ( id == R.id.ll_title || id == R.id.iv_folder_arrow) {
            ...
            if (granted) {
                ...
                ActivityCompat.requestPermissions(this,new String[]{Manifest.permission.CAMERA},1235);
            }
            ...
            if(mSelectPhotoList.size() > 0) {
                if(requestCode==102){
                    if(mSelectPhotoList.size()<GalleryFinal.getFunctionConfig().getMaxSize()){
                        Toast.makeText(this,"请选择身份证正反两面照片",Toast.LENGTH_SHORT).show();
                    }else {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if ( keyCode == KeyEvent.KEYCODE_BACK ) {
            if (mLlFolderPanel!=null&& mLlFolderPanel.getVisibility() == View.VISIBLE ) {
                mLlTitle.performClick();
                return true;
            }
        }
